 ...
 * 
 * To-Do
 * This GUI is not user-friendly as it does not handle the following:
 ...
 * - When user clicks on a RoomNode that is finished simulating, it pushes the winner
 * of that round to the next round, resulting in a player playing against itself
 * - Does not check if a level is complete before allowing the user to click on a RoomNode
 * in the level above
 * - Winner of the tournament is displayed as an alert box, which isnt very rewarding for the winner
 * @author zionchilagan
 *
 */
...
public class TournamentPane extends StackPane implements Observer {
	 ...
	 * the winner of the Tournament is displayed in an alert box
	 * 
	 */
	@Override
	public void update() {
	...
	private class SubTree extends StackPane {
		 ...
		 * 
		 * To-Do
		 * - Handle events where user clicks on an already-simulated RoomNode
